...
const SubBoardList = ({ board }: ISubBoardListProps) => {
 // const activeProject: any = useSelector<any,any>(state=> state.activeProject)
  ...
  const moveTicket = (cardId: any, destColumnId: any, index: any) => {
  ...
  const onTicketDrop = (cardId: any, destColumnId: any, index: any) => {
    ...
    moveTicketDrop(cardId, destColumnId, index).then(() => fetchProjectData(dispatch, projectId))
  }

  return (
    <DndProvider backend={HTML5Backend}>
      <div className={classes.boardsContainer}>
        <SubBoard subBoardData={{name: 'Backlog', filterCriteriaLabel: null, wipLimit: null, _id: 'backlog' }} isBacklog={true} index={0} moveTicket={moveTicket} onTicketDrop={onTicketDrop}/>
        {
          board.subBoards.map((subBoard, index) => <SubBoard subBoardData={subBoard} key={subBoard.name} index={index + 1} moveTicket={moveTicket} onTicketDrop={onTicketDrop}/>)
        }
        <SubBoard subBoardData={{name: 'Closed', filterCriteriaLabel: null, wipLimit: null, _id: 'closed' }} showClosed={true} index={10} moveTicket={moveTicket} onTicketDrop={onTicketDrop}/>
      </div>  
  </DndProvider>
  )
}
